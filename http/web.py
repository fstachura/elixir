...
def generate_source(q, project, version, path):
    ...
    from filters.projects import get_common_filters, project_filters
    from filters.utils import FilterContext

    filters = get_common_filters()
    if project in project_filters:
        filters.extend(project_filters[project]())
    ...
    filters = [f for f in filters if f.check_if_applies(filter_ctx)]
