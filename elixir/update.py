from concurrent.futures import ProcessPoolExecutor, ThreadPoolExecutor, as_completed, TimeoutError, wait
...
# ASSUMPTIONS:
# * only one tag is updated at once
...
# Get definitions for a single id
...
# Get references for a single id
def get_refs(idx, hash, filename, defs, refs):
    ...
    for tok in tokens:
        ...
        if even:
            ...
            if deflist and not deflist.exists(idx, line_num):
...
# Get comps docs for a single id
    ...
    def f(future):
        try:
            ...
            if result is not None:
                ...
                print("add_defs handling time", (time.time()-start), len(result))
        except Exception:
    ...
    def f(future):
        try:
            ...
            if result is not None:
                ...
                print("add_refs handling time", (time.time()-start), len(result))
        except Exception:
    ...
    def f(future):
        try:
            ...
            for result in results:
                if result is not None:
                    callback(*result)
            print(callback.__name__, "handling time", (time.time()-start))
        except Exception:
