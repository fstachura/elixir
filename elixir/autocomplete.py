...
from .data import DefList

class AutocompleteResource:
    def on_get(self, req, resp, project, version, ident_prefix):
        ...
        files_this_version = set(id for id, _ in query.db.vers.get(version).iter())
        ...
        key, data = cur.get(query_bytes, DB_SET_RANGE)
        while i <= 10:
            if key.startswith(query_bytes):
                # If found key starts with the prefix, add to response
                # and move to the next key
                def_data = DefList(data)
                for def_file_id, _, _, _ in def_data.iter():
                    if def_file_id in files_this_version:
                        i += 1
