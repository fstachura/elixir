...
def add_defs(db: DB, def_cache: DefCache, defs: DefsDict):
    for ident, occ_list in defs.items():
        ...
        for (idx, type, line, family) in occ_list:
            db.execute("insert into defs (ident, file_id, type, line, family) values (?, ?, ?, ?, ?)",
                       [ident, idx, type, line, family]).fetchone()
...
def add_refs(db: DB, def_cache: DefCache, refs: RefsDict):
    for ident, idx_to_lines in refs.items():
        ...
        for (idx, family), lines in idx_to_lines.items():
            ...
            for line in lines:
                db.execute("insert into refs (ident, file_id, line, family) values (?, ?, ?, ?)",
...
def collect_blobs(db: DB, tag: bytes) -> Dict[int, Tuple[bytes, str]]:
    ...
    for blob in blobs:
        ...
        blob = db.execute("select * from file where hash=?", [hash]).fetchone()
        if not blob:
            file = db.execute("insert into file (hash, path) values (?, ?) returning id", [hash, filename]).fetchone()
            idx_to_hash_and_filename[file[0]] = (hash, filename.decode())
...
def create_database(filename):
    ...
    db.sql("create sequence file_seq start 1")
