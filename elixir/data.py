...
class BsdDB:
    def __init__(self, filename, readonly, contentType=None):
...
class DB:
    def __init__(self, dir, readonly=True, dtscomp=False):
        ...
        self.vars = BsdDB(dir + '/variables.db', ro)
            # Key-value store of basic information
        self.blob = BsdDB(dir + '/blobs.db', ro)
            # Map hash to sequential integer serial number
        self.hash = BsdDB(dir + '/hashes.db', ro)
            # Map serial number back to hash
        self.file = BsdDB(dir + '/filenames.db', ro)
